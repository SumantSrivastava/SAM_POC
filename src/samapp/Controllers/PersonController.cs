...
using samapp.Repositories;

namespace samapp.Controllers;
...
public class PersonsController : ControllerBase
{
    ...
    private readonly IPersonRepository bookRepository;

    public PersonsController(ILogger<BooksController> logger, IPersonRepository bookRepository)
    {
        this.logger = logger;
    ...
    public async Task<ActionResult<IEnumerable<Book>>> Get([FromQuery] int limit = 10)
    {
        ...
        return Ok(await bookRepository.GetPersonsAsync(limit));
    }
    ...
    // POST api/books
    [HttpPost]
    public async Task<ActionResult<Book>> Post([FromBody] Person book)
    {
    ...
    // PUT api/books/5
    [HttpPut("{id}")]
    public async Task<IActionResult> Put(Guid id, [FromBody] Person book)
    {
