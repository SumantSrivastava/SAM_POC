...
namespace samapp.Repositories
{
    ...
    public class PersonRepository : IPersonRepository
    {
        ...
        public PersonRepository(IDynamoDBContext context, ILogger<PersonRepository> logger)
        {
            this.context = context;
            this.logger = logger;
            //DynamoDB Context is set through Dependency Injection
        }

        public async Task<bool> CreateAsync(Person book)
        {
            try
            {
        ...
        public async Task<bool> DeleteAsync(Person book)
        {
            bool result;
            try
            {
        ...
        public async Task<bool> UpdateAsync(Person book)
        {
        ...
        public async Task<Person?> GetByIdAsync(Guid id)
        {
            try
            {
                return await context.LoadAsync<Person>(id);
            }
            catch (Exception ex)
            {
                logger.LogError(ex, "fail to update book from DynamoDb Table");
        ...
        public async Task<IList<Person>> GetPersonsAsync(int limit = 10)
        {
            ...
            try
            {
                ...
                var queryResult = context.FromScanAsync<Person>(scanConfig);
